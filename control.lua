...
local function message_tool_selected(event)
  ...
  --player.print("Please click somewhere on the map.")
end
...
local function get_cursor_position(event)
  ...
  if player.cursor_stack and player.cursor_stack.valid_for_read then
    ...
    --game.print(position.x .. ", " .. position.y)
...
local function place_note(event)
  ...
  if event.element.name == "submit_message_button" then
    ...
    --if global.message_number == nil then global.message_number = 0 end
    
    --global.message_number = global.message_number + 1
    ...
    --table.insert(global.messages, {id = id, position = position, message = message})
    global.messages[id] = {
      id = id,
      position = position,
      message = message
  ...
  elseif event.element.name == "remove_messages_button" then
    ...
    for _, messageunit in pairs(global.messages) do
      if messageunit ~= nil then
        ...
        --game.print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%" .. id)
        --game.print(id)
        --game.print("______________________________________")
...
--script.on_event({ defines.events.on_lua_shortcut, "hk-add-msg-sc" }, place_note(event))
